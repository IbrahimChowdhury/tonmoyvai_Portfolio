...
export function EditBlog({data,refreshData}) {
...
let EditBlog=async(e)=>{
  try {
    e.preventDefault()
    await axios.put(`/editBlog/${data._id}`,{
        title,
        imageLinks,
        desc,
        dateAndTime
  ...
  return (
    <>
      <p className=' bg-blue-gray-50 p-1  ease-in-out duration-200  rounded-md hover:bg-gray-700 hover:text-gray-200 hover:cursor-pointer' onClick={handleOpen}><AiFillEdit/></p>
      <Dialog
        size="xs"
        open={open}
        handler={handleOpen}
      ...
      >
        <form onSubmit={EditBlog} className="mx-auto w-full rounded-lg max-w-[24rem]">
          <CardHeader
            variant="gradient"
            color="blue"
